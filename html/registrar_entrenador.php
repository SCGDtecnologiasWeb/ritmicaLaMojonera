...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
  ...
  if (mysqli_stmt_num_rows($stmt) >= 1) {
    $email_err = "Este correo ya esta siendo usado";
  }
  ...
  if (isset($contraseña)) {
    if (strlen($contraseña) == 0) {
      $password_err .= "No has introducido una contraseña<br>";
    }
  ...
  if (empty($email_err) && empty($password_err) && empty($name_err) && empty($dni_err) && empty($phone_err) && empty($img_err)) {
    ...
    mysqli_stmt_bind_param($stmt, "sssss", $correo, $nombre, $hash, $dni, $telefono);
    mysqli_stmt_execute($stmt);
    mysqli_stmt_close($stmt);
      ...
      <div class="form-container">
        <form action="/html/registrar_entrenador.php" method="POST" enctype="multipart/form-data">
          ...
          <div class=" mb-3">
            ...
            <input class="form-control <?php if (!empty($password_err)) echo "is-invalid" ?>" type="password" id="password" name="password" required>
            <div class="invalid-feedback">
