...
if ($_SERVER["REQUEST_METHOD"] !== "POST") {
  ...
  $sql_get_gimnasta = "SELECT `dni`, `nombreCompleto`, `fechaNacimiento`, `nombreTutor`, `telefono`, `nivel`, `consentimientoFotos`, `alergias`
                       FROM `Gimnasta`
                       WHERE `dni` = (?)";
  ...
  mysqli_stmt_bind_result($stmt, $dni, $nombre, $fechaNac, $tutor, $telefono, $nivel, $consentimiento, $alergias);
  mysqli_stmt_fetch($stmt);
  mysqli_stmt_close($stmt);
  ...
  if (empty($name_err) && empty($birthdate_err) && empty($dni_err) && empty($parent_err) && empty($phone_err) && empty($level_err) && empty($allergies_err) && empty($consent_err) && empty($img_err)) {
    ...
    $sql_update = "UPDATE `Gimnasta` SET `dni` = (?), `nombreCompleto` = (?), `fechaNacimiento` = (?), `nombreTutor` = (?), `telefono` = (?), `nivel` = (?), `consentimientoFotos` = (?), `alergias` = (?), `pago` = (?) WHERE `dni` = (?)";
    $stmt = mysqli_prepare($link, $sql_update);
    mysqli_stmt_bind_param($stmt, "ssssssssss", $dni, $nombre, $fechaNac, $tutor, $telefono, $nivel, $consentimiento, $alergias, $pago, $_GET["idGimnasta"]);
    mysqli_stmt_execute($stmt);
    mysqli_stmt_close($stmt);
...
<body>
  ...
  <div class="main">
    <div class="content-container">
      <h1>Modificar datos de gimnasta</h1>
        ...
        <form action="/html/modificar_gimnasta.php?idGimnasta=<?php echo $_GET["idGimnasta"] ?>" method="POST" enctype="multipart/form-data">
          ...
          <div class="mb-3">
            ...
            <div class="form-check">
              <input class="form-check-input" type="radio" name="level" id="escuela" value="escuela" required <?php if (strtolower($nivel) === "escuela") echo "checked" ?>>
              <label class="form-check-label" for="escuela">Escuela</label>
            </div>
            <div class="form-check">
              <input class="form-check-input" type="radio" name="level" id="federada" value="federada" <?php if (strtolower($nivel) === "federada") echo "checked" ?>>
          ...
          if ($pago === "No") {
            ...
            echo "\" id=\"payment\" name=\"payment\" autocomplete=\"off\" accept=\"image/png, image/jpeg, image/webp\">";
